`ifndef _SYSCTRL_AGENT_SV_
`define _SYSCTRL_AGENT_SV_

//=========================================================================
// sysctrl_agent: sysctrl UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class sysctrl_agent extends uvm_agent;

	// is_active 控制 agent 工作模式（active=可驱动，passive=纯监控）
	uvm_active_passive_enum is_active = UVM_ACTIVE;

	// 子组件句柄
	sysctrl_driver    sysctrl_drv;
	sysctrl_sequencer sysctrl_seqr;
	sysctrl_monitor   sysctrl_mon;

	// agent callback 池（供用户注册扩展）
	`uvm_register_cb(sysctrl_agent, sysctrl_agent_callback)

	`uvm_component_utils_begin(sysctrl_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(sysctrl_drv,  UVM_ALL_ON)
		`uvm_field_object(sysctrl_mon,  UVM_ALL_ON)
		`uvm_field_object(sysctrl_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="sysctrl_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    sysctrl_drv  = sysctrl_driver::type_id::create("sysctrl_drv", this);
		    sysctrl_seqr = sysctrl_sequencer::type_id::create("sysctrl_seqr", this);
		end
		sysctrl_mon = sysctrl_monitor::type_id::create("sysctrl_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    sysctrl_drv.seq_item_port.connect(sysctrl_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    sysctrl_drv.rsp_port.connect(sysctrl_seqr.rsp_export);
		end
...
endclass : sysctrl_agent

	//=========================================================================
	// sysctrl_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
	class sysctrl_agent_callback extends uvm_callback;
		`uvm_object_utils(sysctrl_agent_callback)
		function new(string name="sysctrl_agent_callback");
			super.new(name);
		endfunction
	endclass : sysctrl_agent_callback
