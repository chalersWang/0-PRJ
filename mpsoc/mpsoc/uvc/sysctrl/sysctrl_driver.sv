`ifndef _SYSCTRL_DRIVER_SV_
`define _SYSCTRL_DRIVER_SV_

//=========================================================================
// sysctrl_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class sysctrl_driver extends uvm_driver#(sysctrl_trans);
	...
	function new(string name="sysctrl_driver",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual sysctrl_vif)::get(this,"","sysctrl_vif",vif))
		    `uvm_fatal("sysctrl_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	virtual task reset_phase(uvm_phase phase);
		...
		vif.i_pad_clk = 'd0;
		vif.i_pad_rst_b = 'd0;
		vif.i_pad_boot_mode = 'd0;
		vif.i_pad_host_if_mode = 'd0;
		vif.i_pad_bypass_secure = 'd0;
		vif.o_pad_pn_sync = 'd0;
		vif.i_pad_test_mode = 'd0;
	...
	//=========================================================================
	virtual task driver_one_pkt(sysctrl_trans tr);
...
endclass : sysctrl_driver
	...
	//=========================================================================
	// sysctrl_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends sysctrl_driver_callback;
	...
	class sysctrl_driver_callback extends uvm_callback;
		...
		virtual function void pre_driver(sysctrl_driver drv, sysctrl_trans tr);
		endfunction
