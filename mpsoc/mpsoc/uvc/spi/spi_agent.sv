`ifndef _SPI_AGENT_SV_
`define _SPI_AGENT_SV_

//=========================================================================
// spi_agent: spi UVC 的顶层 agent
...
//	`uvm_register_cb(spi_agent, spi_agent_callback)

	`uvm_component_utils_begin(spi_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(spi_drv,  UVM_ALL_ON)
		`uvm_field_object(spi_mon,  UVM_ALL_ON)
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    spi_drv.rsp_port.connect(spi_seqr.rsp_export);
		end
...
endclass : spi_agent
	...
	//=========================================================================
	// spi_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class spi_agent_callback extends uvm_callback;
		`uvm_object_utils(spi_agent_callback)
		function new(string name="spi_agent_callback");
			super.new(name);
		endfunction
	endclass : spi_agent_callback
*/
