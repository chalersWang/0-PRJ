`ifndef _SPI_SEQUENCE_LIB_SV_
`define _SPI_SEQUENCE_LIB_SV_

//=========================================================================
// spi_base_sequence: spi UVC 的基类 sequence
//   继承自 uvm_sequence，所有 spi 相关 sequence 应继承此类
//=========================================================================
class spi_base_sequence extends uvm_sequence#(spi_trans);
	...
	virtual task pre_body();
		...
		// 获取 config（若不需要可注释）
		if (!uvm_config_db#spi_config::get(null, get_full_name(), "spi_config", spi_cfg))
			`uvm_fatal(get_type_name(), "can not get spi_config object !!!")
...
//=========================================================================
// spi_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class spi_demo_sequence extends spi_base_sequence;
	`uvm_object_utils(spi_demo_sequence)
	function new(string name="spi_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
