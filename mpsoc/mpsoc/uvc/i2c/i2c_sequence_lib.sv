`ifndef _I2C_SEQUENCE_LIB_SV_
`define _I2C_SEQUENCE_LIB_SV_

//=========================================================================
// i2c_base_sequence: i2c UVC 的基类 sequence
//   继承自 uvm_sequence，所有 i2c 相关 sequence 应继承此类
//=========================================================================
class i2c_base_sequence extends uvm_sequence#(i2c_trans);
	...
	virtual task pre_body();
		...
		// 获取 config（若不需要可注释）
		if (!uvm_config_db#i2c_config::get(null, get_full_name(), "i2c_config", i2c_cfg))
			`uvm_fatal(get_type_name(), "can not get i2c_config object !!!")
	...
	virtual task body();
		...
		//
		// 方式3: 手动控制（需要精细控制时）
		// req = i2c_trans::type_id::create("req");
		// start_item(req);
...
//=========================================================================
// i2c_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class i2c_demo_sequence extends i2c_base_sequence;
	`uvm_object_utils(i2c_demo_sequence)
	function new(string name="i2c_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
