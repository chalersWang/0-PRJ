`ifndef _I2C_AGENT_SV_
`define _I2C_AGENT_SV_

//=========================================================================
// i2c_agent: i2c UVC 的顶层 agent
...
//	`uvm_register_cb(i2c_agent, i2c_agent_callback)

	`uvm_component_utils_begin(i2c_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(i2c_drv,  UVM_ALL_ON)
		`uvm_field_object(i2c_mon,  UVM_ALL_ON)
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    i2c_drv.rsp_port.connect(i2c_seqr.rsp_export);
		end
...
endclass : i2c_agent
	...
	//=========================================================================
	// i2c_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class i2c_agent_callback extends uvm_callback;
		`uvm_object_utils(i2c_agent_callback)
		function new(string name="i2c_agent_callback");
			super.new(name);
		endfunction
	endclass : i2c_agent_callback
*/
