`ifndef _QSPI_DRIVER_SV_
`define _QSPI_DRIVER_SV_

//=========================================================================
// qspi_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class qspi_driver extends uvm_driver#(qspi_trans);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual qspi_vif)::get(this,"","qspi_vif",vif))
		    `uvm_fatal("qspi_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	//=========================================================================
	virtual task driver_one_pkt(qspi_trans tr);
...
endclass : qspi_driver

	//=========================================================================
	// qspi_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends qspi_driver_callback;
	...
	class qspi_driver_callback extends uvm_callback;
		...
		virtual function void pre_driver(qspi_driver drv, qspi_trans tr);
		endfunction
