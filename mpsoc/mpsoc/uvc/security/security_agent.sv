`ifndef _SECURITY_AGENT_SV_
`define _SECURITY_AGENT_SV_

//=========================================================================
// security_agent: security UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class security_agent extends uvm_agent;

	// is_active 控制 agent 工作模式（active=可驱动，passive=纯监控）
	uvm_active_passive_enum is_active = UVM_ACTIVE;

	// 子组件句柄
	security_driver    security_drv;
	security_sequencer security_seqr;
	security_monitor   security_mon;

	// agent callback 池（供用户注册扩展）
//	`uvm_register_cb(security_agent, security_agent_callback)

	`uvm_component_utils_begin(security_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(security_drv,  UVM_ALL_ON)
		`uvm_field_object(security_mon,  UVM_ALL_ON)
		`uvm_field_object(security_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="security_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    security_drv  = security_driver::type_id::create("security_drv", this);
		    security_seqr = security_sequencer::type_id::create("security_seqr", this);
		end
		security_mon = security_monitor::type_id::create("security_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    security_drv.seq_item_port.connect(security_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    security_drv.rsp_port.connect(security_seqr.rsp_export);
		end
...
endclass : security_agent

	//=========================================================================
	// security_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class security_agent_callback extends uvm_callback;
		`uvm_object_utils(security_agent_callback)
		function new(string name="security_agent_callback");
			super.new(name);
		endfunction
	endclass : security_agent_callback
*/
