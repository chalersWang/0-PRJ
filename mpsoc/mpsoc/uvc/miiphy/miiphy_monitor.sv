`ifndef _MIIPHY_MONITOR_SV_
`define _MIIPHY_MONITOR_SV_

//=========================================================================
// miiphy_monitor: 从接口采集 transaction，通过 analysis_port 广播
//   使用 run_phase 持续监控（监控类组件适合 run_phase，因其需全仿真期间运行）
//   内建 monitor_callback 支持用户扩展
//=========================================================================
class miiphy_monitor extends uvm_monitor;
	...
	function new(string name="miiphy_monitor",uvm_component parent=null);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual miiphy_vif)::get(this,"","miiphy_vif",vif))
		    `uvm_fatal("miiphy_monitor","virtual interface must be set for it!!!")
	...
	*/
	...
	//   X/Z 检查：通过 `ifdef CHECK_SIGNAL_XZ_MIIPHY 宏控制
	//   覆盖率采集：通过 `ifdef COVERAGE_MIIPHY 宏控制
	...
	virtual task run_phase(uvm_phase phase);
		...
		forever begin
			...
			`ifdef CHECK_SIGNAL_XZ_MIIPHY
				// 检查所有信号是否存在 X 或 Z 状态
				if($isunknown(vif.phy_rxd_i)==1)
					`uvm_error(get_type_name(), $sformatf("signal:phy_rxd_i is X/Z at %0t", $time))
				if($isunknown(vif.phy_rxdv_i)==1)
					`uvm_error(get_type_name(), $sformatf("signal:phy_rxdv_i is X/Z at %0t", $time))
				if($isunknown(vif.phy_rxer_i)==1)
					`uvm_error(get_type_name(), $sformatf("signal:phy_rxer_i is X/Z at %0t", $time))
				if($isunknown(vif.phy_txd_o)==1)
					`uvm_error(get_type_name(), $sformatf("signal:phy_txd_o is X/Z at %0t", $time))
				if($isunknown(vif.phy_txen_o)==1)
					`uvm_error(get_type_name(), $sformatf("signal:phy_txen_o is X/Z at %0t", $time))
				if($isunknown(vif.RGMIIRXC_i)==1)
					`uvm_error(get_type_name(), $sformatf("signal:RGMIIRXC_i is X/Z at %0t", $time))
				if($isunknown(vif.RGMIITXC_o)==1)
					`uvm_error(get_type_name(), $sformatf("signal:RGMIITXC_o is X/Z at %0t", $time))
				if($isunknown(vif.phy_link_i)==1)
					`uvm_error(get_type_name(), $sformatf("signal:phy_link_i is X/Z at %0t", $time))
			`endif

			// --- 3. 创建 transaction 并采样接口信号 ---
			miiphy_tr = miiphy_trans::type_id::create("miiphy_tr");
			miiphy_tr.phy_rxd_i = vif.phy_rxd_i;
			miiphy_tr.phy_rxdv_i = vif.phy_rxdv_i;
			miiphy_tr.phy_rxer_i = vif.phy_rxer_i;
			miiphy_tr.phy_txd_o = vif.phy_txd_o;
			miiphy_tr.phy_txen_o = vif.phy_txen_o;
			miiphy_tr.RGMIIRXC_i = vif.RGMIIRXC_i;
			miiphy_tr.RGMIITXC_o = vif.RGMIITXC_o;
...
endclass : miiphy_monitor
	...
	class miiphy_monitor_callback extends uvm_callback;
		...
		virtual function void pre_collect(miiphy_monitor mon, miiphy_trans tr);
		endfunction
