`ifndef _PN_IRT_AGENT_SV_
`define _PN_IRT_AGENT_SV_

//=========================================================================
// pn_irt_agent: pn_irt UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class pn_irt_agent extends uvm_agent;

	// is_active 控制 agent 工作模式（active=可驱动，passive=纯监控）
	uvm_active_passive_enum is_active = UVM_ACTIVE;

	// 子组件句柄
	pn_irt_driver    pn_irt_drv;
	pn_irt_sequencer pn_irt_seqr;
	pn_irt_monitor   pn_irt_mon;

	// agent callback 池（供用户注册扩展）
//	`uvm_register_cb(pn_irt_agent, pn_irt_agent_callback)

	`uvm_component_utils_begin(pn_irt_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(pn_irt_drv,  UVM_ALL_ON)
		`uvm_field_object(pn_irt_mon,  UVM_ALL_ON)
		`uvm_field_object(pn_irt_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="pn_irt_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    pn_irt_drv  = pn_irt_driver::type_id::create("pn_irt_drv", this);
		    pn_irt_seqr = pn_irt_sequencer::type_id::create("pn_irt_seqr", this);
		end
		pn_irt_mon = pn_irt_monitor::type_id::create("pn_irt_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    pn_irt_drv.seq_item_port.connect(pn_irt_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    pn_irt_drv.rsp_port.connect(pn_irt_seqr.rsp_export);
		end
...
endclass : pn_irt_agent

	//=========================================================================
	// pn_irt_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class pn_irt_agent_callback extends uvm_callback;
		`uvm_object_utils(pn_irt_agent_callback)
		function new(string name="pn_irt_agent_callback");
			super.new(name);
		endfunction
	endclass : pn_irt_agent_callback
*/
