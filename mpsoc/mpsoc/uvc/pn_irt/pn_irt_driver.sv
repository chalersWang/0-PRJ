`ifndef _PN_IRT_DRIVER_SV_
`define _PN_IRT_DRIVER_SV_

//=========================================================================
// pn_irt_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class pn_irt_driver extends uvm_driver#(pn_irt_trans);

	virtual pn_irt_vif    vif;
	// driver callback 池，允许用户注册回调扩展 driver 行为
//	`uvm_register_cb(pn_irt_driver, pn_irt_driver_callback)

	`uvm_component_utils(pn_irt_driver)

	function new(string name="pn_irt_driver",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual pn_irt_vif)::get(this,"","pn_irt_vif",vif))
		    `uvm_fatal("pn_irt_driver","virtual interface must be set for it!!!")
	...
	virtual task main_phase(uvm_phase phase);
		...
		while(1) begin
			...
			if (req == null) begin
				...
				// 调用回调：pre_driver
//				`uvm_do_callbacks(pn_irt_driver, pn_irt_driver_callback, pre_driver(this, req))
				// --- 驱动 transaction ---
				driver_one_pkt(req);
				// --- 驱动完成 ---
				// 调用回调：post_driver
//				`uvm_do_callbacks(pn_irt_driver, pn_irt_driver_callback, post_driver(this, req))
	...
	//=========================================================================
	virtual task driver_one_pkt(pn_irt_trans tr);
...
endclass : pn_irt_driver

	//=========================================================================
	// pn_irt_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends pn_irt_driver_callback;
	//       function void pre_driver(...); ... endfunction
	//     endclass
	//     pn_irt_driver_callback::add(drv, my_cb);
	//=========================================================================
/*
	class pn_irt_driver_callback extends uvm_callback;
		`uvm_object_utils(pn_irt_driver_callback)
		function new(string name="pn_irt_driver_callback");
			super.new(name);
		endfunction

		// 在 driver 驱动前调用，可修改 transaction 内容
		virtual function void pre_driver(pn_irt_driver drv, pn_irt_trans tr);
		endfunction

		// 在 driver 驱动后调用，可检查驱动结果
		virtual function void post_driver(pn_irt_driver drv, pn_irt_trans tr);
		endfunction
	endclass : pn_irt_driver_callback
*/
