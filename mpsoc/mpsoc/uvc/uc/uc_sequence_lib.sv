`ifndef _UC_SEQUENCE_LIB_SV_
`define _UC_SEQUENCE_LIB_SV_

//=========================================================================
// uc_base_sequence: uc UVC 的基类 sequence
//   继承自 uvm_sequence，所有 uc 相关 sequence 应继承此类
//=========================================================================
class uc_base_sequence extends uvm_sequence#(uc_trans);

	uc_config uc_cfg;

	`uvm_object_utils(uc_base_sequence)

	function new(string name="uc_base_sequence");
		super.new(name);
	endfunction
	...
	virtual task body();
		...
		//
		// 方式3: 手动控制（需要精细控制时）
		// req = uc_trans::type_id::create("req");
		// start_item(req);
	...
	endtask : post_body
endclass : uc_base_sequence

//=========================================================================
// uc_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class uc_demo_sequence extends uc_base_sequence;
	`uvm_object_utils(uc_demo_sequence)
	function new(string name="uc_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
