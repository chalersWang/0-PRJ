`ifndef _UC_AGENT_SV_
`define _UC_AGENT_SV_

//=========================================================================
// uc_agent: uc UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class uc_agent extends uvm_agent;
...
//	`uvm_register_cb(uc_agent, uc_agent_callback)

	`uvm_component_utils_begin(uc_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(uc_drv,  UVM_ALL_ON)
		`uvm_field_object(uc_mon,  UVM_ALL_ON)
		`uvm_field_object(uc_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="uc_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    uc_drv  = uc_driver::type_id::create("uc_drv", this);
		    uc_seqr = uc_sequencer::type_id::create("uc_seqr", this);
		end
		uc_mon = uc_monitor::type_id::create("uc_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    uc_drv.seq_item_port.connect(uc_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    uc_drv.rsp_port.connect(uc_seqr.rsp_export);
		end
...
endclass : uc_agent
	...
	//=========================================================================
	// uc_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class uc_agent_callback extends uvm_callback;
		`uvm_object_utils(uc_agent_callback)
		function new(string name="uc_agent_callback");
			super.new(name);
		endfunction
	endclass : uc_agent_callback
*/
