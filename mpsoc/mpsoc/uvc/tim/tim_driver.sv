`ifndef _TIM_DRIVER_SV_
`define _TIM_DRIVER_SV_

//=========================================================================
// tim_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class tim_driver extends uvm_driver#(tim_trans);

	virtual tim_vif    vif;
	// driver callback 池，允许用户注册回调扩展 driver 行为
//	`uvm_register_cb(tim_driver, tim_driver_callback)

	`uvm_component_utils(tim_driver)

	function new(string name="tim_driver",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual tim_vif)::get(this,"","tim_vif",vif))
		    `uvm_fatal("tim_driver","virtual interface must be set for it!!!")
	...
	virtual task main_phase(uvm_phase phase);
		...
		while(1) begin
			...
			if (req == null) begin
				...
				// 调用回调：pre_driver
//				`uvm_do_callbacks(tim_driver, tim_driver_callback, pre_driver(this, req))
				// --- 驱动 transaction ---
				driver_one_pkt(req);
				// --- 驱动完成 ---
				// 调用回调：post_driver
//				`uvm_do_callbacks(tim_driver, tim_driver_callback, post_driver(this, req))
	...
	// tim_in 为定时器外部输入（可作时钟源/捕获触发），driver 产生脉冲激励
	...
	//=========================================================================
		virtual task driver_one_pkt(tim_trans tr);
		`uvm_info(get_type_name(), $sformatf("tim: ch=%0d pulses=%0d period=%0d",
			tr.channel, tr.pulse_count, tr.pulse_period), UVM_HIGH)
		for (int p = 0; p < tr.pulse_count; p++) begin
			vif.tim_in[tr.channel] = 1'b1;
			repeat(tr.pulse_period) @(posedge vif.clk);
			vif.tim_in[tr.channel] = 1'b0;
			repeat(tr.pulse_period) @(posedge vif.clk);
		end
...
endclass : tim_driver
	...
	//=========================================================================
	// tim_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends tim_driver_callback;
	//       function void pre_driver(...); ... endfunction
	//     endclass
	//     tim_driver_callback::add(drv, my_cb);
	//=========================================================================
/*
	class tim_driver_callback extends uvm_callback;
		`uvm_object_utils(tim_driver_callback)
		function new(string name="tim_driver_callback");
			super.new(name);
		endfunction

		// 在 driver 驱动前调用，可修改 transaction 内容
		virtual function void pre_driver(tim_driver drv, tim_trans tr);
		endfunction

		// 在 driver 驱动后调用，可检查驱动结果
		virtual function void post_driver(tim_driver drv, tim_trans tr);
		endfunction
	endclass : tim_driver_callback
*/
