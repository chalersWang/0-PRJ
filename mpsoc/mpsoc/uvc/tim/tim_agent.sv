`ifndef _TIM_AGENT_SV_
`define _TIM_AGENT_SV_

//=========================================================================
// tim_agent: tim UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class tim_agent extends uvm_agent;
...
//	`uvm_register_cb(tim_agent, tim_agent_callback)

	`uvm_component_utils_begin(tim_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(tim_drv,  UVM_ALL_ON)
		`uvm_field_object(tim_mon,  UVM_ALL_ON)
		`uvm_field_object(tim_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="tim_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    tim_drv  = tim_driver::type_id::create("tim_drv", this);
		    tim_seqr = tim_sequencer::type_id::create("tim_seqr", this);
		end
		tim_mon = tim_monitor::type_id::create("tim_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    tim_drv.seq_item_port.connect(tim_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    tim_drv.rsp_port.connect(tim_seqr.rsp_export);
		end
...
endclass : tim_agent
	...
	//=========================================================================
	// tim_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class tim_agent_callback extends uvm_callback;
		`uvm_object_utils(tim_agent_callback)
		function new(string name="tim_agent_callback");
			super.new(name);
		endfunction
	endclass : tim_agent_callback
*/
