`ifndef _WDT_DRIVER_SV_
`define _WDT_DRIVER_SV_

//=========================================================================
// wdt_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class wdt_driver extends uvm_driver#(wdt_trans);

	virtual wdt_vif    vif;
	// driver callback 池，允许用户注册回调扩展 driver 行为
//	`uvm_register_cb(wdt_driver, wdt_driver_callback)

	`uvm_component_utils(wdt_driver)

	function new(string name="wdt_driver",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual wdt_vif)::get(this,"","wdt_vif",vif))
		    `uvm_fatal("wdt_driver","virtual interface must be set for it!!!")
	...
	virtual task main_phase(uvm_phase phase);
		...
		while(1) begin
			...
			if (req == null) begin
				...
				// 调用回调：pre_driver
//				`uvm_do_callbacks(wdt_driver, wdt_driver_callback, pre_driver(this, req))
				// --- 驱动 transaction ---
				driver_one_pkt(req);
				// --- 驱动完成 ---
				// 调用回调：post_driver
//				`uvm_do_callbacks(wdt_driver, wdt_driver_callback, post_driver(this, req))
	...
	// WDT 无外部输入引脚：喂狗/配置均经 APB 寄存器（超出信号级 UVC 范围）
	// driver 的职责退化为：等待并观察中断/复位事件，驱动信号为空操作。
	...
	//=========================================================================
		virtual task driver_one_pkt(wdt_trans tr);
		`uvm_info(get_type_name(), $sformatf("wdt: 观察 %0d 周期内的 int/rst 事件", tr.timeout_cycles), UVM_HIGH)
		tr.int_seen = 0;
		tr.rst_seen = 0;
		fork
			begin
				wait(vif.wdt_int === 1'b1);
				tr.int_seen = 1;
			end
			begin
				wait(vif.wdt_rst === 1'b1);
				tr.rst_seen = 1;
			end
			begin
				repeat(tr.timeout_cycles) @(posedge vif.clk);
			end
		join_any
		disable fork;
		`uvm_info(get_type_name(), $sformatf("wdt: int_seen=%0b rst_seen=%0b", tr.int_seen, tr.rst_seen), UVM_HIGH)
...
endclass : wdt_driver
	...
	//=========================================================================
	// wdt_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends wdt_driver_callback;
	//       function void pre_driver(...); ... endfunction
	//     endclass
	//     wdt_driver_callback::add(drv, my_cb);
	//=========================================================================
/*
	class wdt_driver_callback extends uvm_callback;
		`uvm_object_utils(wdt_driver_callback)
		function new(string name="wdt_driver_callback");
			super.new(name);
		endfunction

		// 在 driver 驱动前调用，可修改 transaction 内容
		virtual function void pre_driver(wdt_driver drv, wdt_trans tr);
		endfunction

		// 在 driver 驱动后调用，可检查驱动结果
		virtual function void post_driver(wdt_driver drv, wdt_trans tr);
		endfunction
	endclass : wdt_driver_callback
*/
