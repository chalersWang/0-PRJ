`ifndef _WDT_MONITOR_SV_
`define _WDT_MONITOR_SV_

//=========================================================================
// wdt_monitor: 从接口采集 transaction，通过 analysis_port 广播
...
class wdt_monitor extends uvm_monitor;
	...
	uvm_analysis_port #(wdt_trans)    mon_analysis_port;
//	`uvm_register_cb(wdt_monitor, wdt_monitor_callback)

	`uvm_component_utils(wdt_monitor)

	function new(string name="wdt_monitor",uvm_component parent=null);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual wdt_vif)::get(this,"","wdt_vif",vif))
		    `uvm_fatal("wdt_monitor","virtual interface must be set for it!!!")
	...
	*/
	...
	//   X/Z 检查：通过 `ifdef CHECK_SIGNAL_XZ_WDT 宏控制
	//   覆盖率采集：通过 `ifdef COVERAGE_WDT 宏控制
	//   注意：原来使用 forever fork join，会导致每个时钟周期创建一个
	//   永不释放的线程，内存持续增长。已修复为串行 @(posedge vif.clk) 模式。
	//=========================================================================
		virtual task run_phase(uvm_phase phase);
		wdt_trans tr;
		logic prev_int, prev_rst;
		`uvm_info(get_type_name(), "run_phase begin", UVM_MEDIUM)
		prev_int = vif.wdt_int;
		prev_rst = vif.wdt_rst;
		forever begin
			@(posedge vif.clk);
			if (vif.wdt_int && !prev_int) begin
				tr = wdt_trans::type_id::create("wdt_tr");
				tr.int_seen = 1; tr.rst_seen = 0;
				tr.wdt_int = vif.wdt_int; tr.wdt_rst = vif.wdt_rst;
				mon_analysis_port.write(tr);
			end
			if (vif.wdt_rst && !prev_rst) begin
				tr = wdt_trans::type_id::create("wdt_tr");
				tr.int_seen = 0; tr.rst_seen = 1;
				tr.wdt_int = vif.wdt_int; tr.wdt_rst = vif.wdt_rst;
				mon_analysis_port.write(tr);
			end
			prev_int = vif.wdt_int;
...
endclass : wdt_monitor

	//=========================================================================
	// wdt_monitor_callback: Monitor 回调基类
	//   用户可继承此类扩展 monitor 行为（如注入错误、修改采集数据等）
	//=========================================================================
/*
	class wdt_monitor_callback extends uvm_callback;
		`uvm_object_utils(wdt_monitor_callback)
		function new(string name="wdt_monitor_callback");
			super.new(name);
		endfunction

		// 在 monitor 采样后、write 前调用，可修改 transaction
		virtual function void pre_collect(wdt_monitor mon, wdt_trans tr);
		endfunction

		// 在 monitor write 后调用
		virtual function void post_collect(wdt_monitor mon, wdt_trans tr);
		endfunction
	endclass : wdt_monitor_callback
*/
