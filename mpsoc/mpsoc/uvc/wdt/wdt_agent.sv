`ifndef _WDT_AGENT_SV_
`define _WDT_AGENT_SV_

//=========================================================================
// wdt_agent: wdt UVC 的顶层 agent
...
//	`uvm_register_cb(wdt_agent, wdt_agent_callback)

	`uvm_component_utils_begin(wdt_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(wdt_drv,  UVM_ALL_ON)
		`uvm_field_object(wdt_mon,  UVM_ALL_ON)
		`uvm_field_object(wdt_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="wdt_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    wdt_seqr = wdt_sequencer::type_id::create("wdt_seqr", this);
		end
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    wdt_drv.seq_item_port.connect(wdt_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    wdt_drv.rsp_port.connect(wdt_seqr.rsp_export);
		end
...
endclass : wdt_agent
	...
	//=========================================================================
	// wdt_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class wdt_agent_callback extends uvm_callback;
		`uvm_object_utils(wdt_agent_callback)
		function new(string name="wdt_agent_callback");
			super.new(name);
		endfunction
	endclass : wdt_agent_callback
*/
