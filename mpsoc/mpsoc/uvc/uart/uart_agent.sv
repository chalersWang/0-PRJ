`ifndef _UART_AGENT_SV_
`define _UART_AGENT_SV_

//=========================================================================
// uart_agent: uart UVC 的顶层 agent
...
class uart_agent extends uvm_agent;
	...
	`uvm_register_cb(uart_agent, uart_agent_callback)
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    uart_drv.rsp_port.connect(uart_seqr.rsp_export);
		end
...
endclass : uart_agent
	...
	//=========================================================================
	// uart_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
	class uart_agent_callback extends uvm_callback;
		`uvm_object_utils(uart_agent_callback)
		function new(string name="uart_agent_callback");
			super.new(name);
		endfunction
	endclass : uart_agent_callback
