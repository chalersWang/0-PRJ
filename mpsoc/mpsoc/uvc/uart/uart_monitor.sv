`ifndef _UART_MONITOR_SV_
`define _UART_MONITOR_SV_

//=========================================================================
// uart_monitor: 从接口采集 transaction，通过 analysis_port 广播
...
class uart_monitor extends uvm_monitor;
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual uart_vif)::get(this,"","uart_vif",vif))
		    `uvm_fatal("uart_monitor","virtual interface must be set for it!!!")
	...
	*/
	...
	//   X/Z 检查：通过 `ifdef CHECK_SIGNAL_XZ_UART 宏控制
	//   覆盖率采集：通过 `ifdef COVERAGE_UART 宏控制
	...
	virtual task run_phase(uvm_phase phase);
		...
		forever begin
			...
			`ifdef CHECK_SIGNAL_XZ_UART
				// 检查所有信号是否存在 X 或 Z 状态
				if($isunknown(vif.i_pad_uart0_sin)==1)
					`uvm_error(get_type_name(), $sformatf("signal:i_pad_uart0_sin is X/Z at %0t", $time))
				if($isunknown(vif.o_pad_uart0_sout)==1)
					`uvm_error(get_type_name(), $sformatf("signal:o_pad_uart0_sout is X/Z at %0t", $time))
			`endif

			// --- 3. 创建 transaction 并采样接口信号 ---
			uart_tr = uart_trans::type_id::create("uart_tr");
			uart_tr.i_pad_uart0_sin = vif.i_pad_uart0_sin;
...
endclass : uart_monitor
	...
	class uart_monitor_callback extends uvm_callback;
		...
		virtual function void pre_collect(uart_monitor mon, uart_trans tr);
		endfunction
