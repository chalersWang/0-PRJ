`ifndef _UART_SEQUENCE_LIB_SV_
`define _UART_SEQUENCE_LIB_SV_

//=========================================================================
// uart_base_sequence: uart UVC 的基类 sequence
//   继承自 uvm_sequence，所有 uart 相关 sequence 应继承此类
//=========================================================================
class uart_base_sequence extends uvm_sequence#(uart_trans);
	...
	virtual task pre_body();
		...
		// 获取 config（若不需要可注释）
		if (!uvm_config_db#(uart_config)::get(null, get_full_name(), "uart_config", uart_cfg))
			`uvm_fatal(get_type_name(), "can not get uart_config object !!!")
...
//=========================================================================
// uart_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class uart_demo_sequence extends uart_base_sequence;
	`uvm_object_utils(uart_demo_sequence)
	function new(string name="uart_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
