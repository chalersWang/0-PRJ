...
//=========================================================================
// esc_vif: esc UVC 的 virtual interface
//   interface 端口：clk, rstn → 由 tb_top 传入
//   内部信号：使用 logic 类型（非 input/output），由 DUT 和 driver 共驱
//=========================================================================
interface esc_vif(input logic clk, input logic rstn);

	// ===== DUT 信号声明（logic 类型） =====
		logic ecat_mii_rx_clk;
		logic ecat_mii_rx_dv;
		logic [3:0] ecat_mii_rxd;
		logic ecat_mii_tx_clk;
		logic ecat_mii_tx_en;
		logic [3:0] ecat_mii_txd;
		logic ecat_sync0;
		logic ecat_sync1;
