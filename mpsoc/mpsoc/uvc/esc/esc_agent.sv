`ifndef _ESC_AGENT_SV_
`define _ESC_AGENT_SV_

//=========================================================================
// esc_agent: esc UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class esc_agent extends uvm_agent;
...
//	`uvm_register_cb(esc_agent, esc_agent_callback)

	`uvm_component_utils_begin(esc_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(esc_drv,  UVM_ALL_ON)
		`uvm_field_object(esc_mon,  UVM_ALL_ON)
		`uvm_field_object(esc_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="esc_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    esc_drv  = esc_driver::type_id::create("esc_drv", this);
		    esc_seqr = esc_sequencer::type_id::create("esc_seqr", this);
		end
		esc_mon = esc_monitor::type_id::create("esc_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    esc_drv.seq_item_port.connect(esc_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    esc_drv.rsp_port.connect(esc_seqr.rsp_export);
		end
...
endclass : esc_agent
	...
	//=========================================================================
	// esc_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class esc_agent_callback extends uvm_callback;
		`uvm_object_utils(esc_agent_callback)
		function new(string name="esc_agent_callback");
			super.new(name);
		endfunction
	endclass : esc_agent_callback
*/
