`ifndef _JTAG_SEQUENCE_LIB_SV_
`define _JTAG_SEQUENCE_LIB_SV_

//=========================================================================
// jtag_base_sequence: jtag UVC 的基类 sequence
//   继承自 uvm_sequence，所有 jtag 相关 sequence 应继承此类
//=========================================================================
class jtag_base_sequence extends uvm_sequence#(jtag_trans);
	...
	virtual task pre_body();
		...
		// 获取 config（若不需要可注释）
		if (!uvm_config_db#(jtag_config)::get(null, get_full_name(), "jtag_config", jtag_cfg))
			`uvm_fatal(get_type_name(), "can not get jtag_config object !!!")
	...
	virtual task body();
		...
		//
		// 方式3: 手动控制（需要精细控制时）
		// req = jtag_trans::type_id::create("req");
		// start_item(req);
...
//=========================================================================
// jtag_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class jtag_demo_sequence extends jtag_base_sequence;
	`uvm_object_utils(jtag_demo_sequence)
	function new(string name="jtag_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
