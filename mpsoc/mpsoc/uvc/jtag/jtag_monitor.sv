`ifndef _JTAG_MONITOR_SV_
`define _JTAG_MONITOR_SV_

//=========================================================================
// jtag_monitor: 从接口采集 transaction，通过 analysis_port 广播
...
class jtag_monitor extends uvm_monitor;
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual jtag_vif)::get(this,"","jtag_vif",vif))
		    `uvm_fatal("jtag_monitor","virtual interface must be set for it!!!")
	...
	*/
	...
	//   X/Z 检查：通过 `ifdef CHECK_SIGNAL_XZ_JTAG 宏控制
	//   覆盖率采集：通过 `ifdef COVERAGE_JTAG 宏控制
	...
	virtual task run_phase(uvm_phase phase);
		...
		forever begin
			...
			`ifdef CHECK_SIGNAL_XZ_JTAG
				// 检查所有信号是否存在 X 或 Z 状态
				if($isunknown(vif.i_pad_jtg_nrst_b)==1)
					`uvm_error(get_type_name(), $sformatf("signal:i_pad_jtg_nrst_b is X/Z at %0t", $time))
				if($isunknown(vif.i_pad_jtg_tclk)==1)
					`uvm_error(get_type_name(), $sformatf("signal:i_pad_jtg_tclk is X/Z at %0t", $time))
				if($isunknown(vif.i_pad_jtg_tdi)==1)
					`uvm_error(get_type_name(), $sformatf("signal:i_pad_jtg_tdi is X/Z at %0t", $time))
				if($isunknown(vif.i_pad_jtg_tms)==1)
					`uvm_error(get_type_name(), $sformatf("signal:i_pad_jtg_tms is X/Z at %0t", $time))
				if($isunknown(vif.i_pad_jtg_trst_b)==1)
					`uvm_error(get_type_name(), $sformatf("signal:i_pad_jtg_trst_b is X/Z at %0t", $time))
				if($isunknown(vif.o_pad_jtg_tdo)==1)
					`uvm_error(get_type_name(), $sformatf("signal:o_pad_jtg_tdo is X/Z at %0t", $time))
			`endif

			// --- 3. 创建 transaction 并采样接口信号 ---
			jtag_tr = jtag_trans::type_id::create("jtag_tr");
			jtag_tr.i_pad_jtg_nrst_b = vif.i_pad_jtg_nrst_b;
			jtag_tr.i_pad_jtg_tclk = vif.i_pad_jtg_tclk;
			jtag_tr.i_pad_jtg_tdi = vif.i_pad_jtg_tdi;
			jtag_tr.i_pad_jtg_tms = vif.i_pad_jtg_tms;
			jtag_tr.i_pad_jtg_trst_b = vif.i_pad_jtg_trst_b;
...
endclass : jtag_monitor
	...
	class jtag_monitor_callback extends uvm_callback;
		...
		virtual function void pre_collect(jtag_monitor mon, jtag_trans tr);
		endfunction
