`ifndef _JTAG_DRIVER_SV_
`define _JTAG_DRIVER_SV_

//=========================================================================
// jtag_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class jtag_driver extends uvm_driver#(jtag_trans);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual jtag_vif)::get(this,"","jtag_vif",vif))
		    `uvm_fatal("jtag_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	virtual task reset_phase(uvm_phase phase);
		...
		vif.i_pad_jtg_nrst_b = 'd0;
		vif.i_pad_jtg_tclk = 'd0;
		vif.i_pad_jtg_tdi = 'd0;
		vif.i_pad_jtg_tms = 'd0;
		vif.i_pad_jtg_trst_b = 'd0;
		vif.o_pad_jtg_tdo = 'd0;
	...
	//=========================================================================
	virtual task driver_one_pkt(jtag_trans tr);
...
endclass : jtag_driver
	...
	//=========================================================================
	// jtag_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends jtag_driver_callback;
	...
	class jtag_driver_callback extends uvm_callback;
		...
		virtual function void pre_driver(jtag_driver drv, jtag_trans tr);
		endfunction
