`ifndef _GPIO_DRIVER_SV_
`define _GPIO_DRIVER_SV_

//=========================================================================
// gpio_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class gpio_driver extends uvm_driver#(gpio_trans);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual gpio_vif)::get(this,"","gpio_vif",vif))
		    `uvm_fatal("gpio_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	virtual task reset_phase(uvm_phase phase);
		...
		vif.b_pad_gpio_porta = 'd0;
		vif.b_pad_gpio_portb = 'd0;
	...
	//=========================================================================
	virtual task driver_one_pkt(gpio_trans tr);
...
endclass : gpio_driver
	...
	//=========================================================================
	// gpio_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends gpio_driver_callback;
	...
	class gpio_driver_callback extends uvm_callback;
		...
		virtual function void pre_driver(gpio_driver drv, gpio_trans tr);
		endfunction
