`ifndef _GPIO_SEQUENCE_LIB_SV_
`define _GPIO_SEQUENCE_LIB_SV_

//=========================================================================
// gpio_base_sequence: gpio UVC 的基类 sequence
//   继承自 uvm_sequence，所有 gpio 相关 sequence 应继承此类
//=========================================================================
class gpio_base_sequence extends uvm_sequence#(gpio_trans);
	...
	virtual task pre_body();
		...
		// 获取 config（若不需要可注释）
		if (!uvm_config_db#(gpio_config)::get(null, get_full_name(), "gpio_config", gpio_cfg))
			`uvm_fatal(get_type_name(), "can not get gpio_config object !!!")
	...
	virtual task body();
		...
		//
		// 方式3: 手动控制（需要精细控制时）
		// req = gpio_trans::type_id::create("req");
		// start_item(req);
	...
	endtask : post_body
endclass : gpio_base_sequence

//=========================================================================
// gpio_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class gpio_demo_sequence extends gpio_base_sequence;
	`uvm_object_utils(gpio_demo_sequence)
	function new(string name="gpio_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
