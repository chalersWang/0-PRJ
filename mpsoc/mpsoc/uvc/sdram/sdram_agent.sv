`ifndef _SDRAM_AGENT_SV_
`define _SDRAM_AGENT_SV_

//=========================================================================
// sdram_agent: sdram UVC 的顶层 agent
...
class sdram_agent extends uvm_agent;
	...
	sdram_monitor   sdram_mon;

	// agent callback 池（供用户注册扩展）
//	`uvm_register_cb(sdram_agent, sdram_agent_callback)

	`uvm_component_utils_begin(sdram_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(sdram_drv,  UVM_ALL_ON)
		`uvm_field_object(sdram_mon,  UVM_ALL_ON)
		`uvm_field_object(sdram_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="sdram_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    sdram_seqr = sdram_sequencer::type_id::create("sdram_seqr", this);
		end
		sdram_mon = sdram_monitor::type_id::create("sdram_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    sdram_drv.rsp_port.connect(sdram_seqr.rsp_export);
		end
...
endclass : sdram_agent
	...
	//=========================================================================
	// sdram_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class sdram_agent_callback extends uvm_callback;
		`uvm_object_utils(sdram_agent_callback)
		function new(string name="sdram_agent_callback");
			super.new(name);
		endfunction
	endclass : sdram_agent_callback
*/
