`ifndef _SWITCH_DRIVER_SV_
`define _SWITCH_DRIVER_SV_

//=========================================================================
// switch_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class switch_driver extends uvm_driver#(switch_trans);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual switch_vif)::get(this,"","switch_vif",vif))
		    `uvm_fatal("switch_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	virtual task reset_phase(uvm_phase phase);
		...
		vif.switch_mii_p0_rxclock = 'd0;
		vif.switch_mii_p0_rxerror = 'd0;
		vif.switch_mii_p0_rxenable = 'd0;
		vif.switch_mii_p0_rx = 'd0;
		vif.switch_mii_p0_txclock = 'd0;
		vif.switch_mii_p0_txenable = 'd0;
		vif.switch_mii_p0_tx = 'd0;
		vif.switch_mii_p0_link = 'd0;
		vif.switch_mii_p1_rxclock = 'd0;
		vif.switch_mii_p1_rxerror = 'd0;
		vif.switch_mii_p1_rxenable = 'd0;
		vif.switch_mii_p1_rx = 'd0;
		vif.switch_mii_p1_txclock = 'd0;
		vif.switch_mii_p1_txenable = 'd0;
		vif.switch_mii_p1_tx = 'd0;
		vif.switch_mii_p1_link = 'd0;
		vif.switch_mdio_clock = 'd0;
		vif.switch_mdio_data = 'd0;
	...
	//=========================================================================
	virtual task driver_one_pkt(switch_trans tr);
...
endclass : switch_driver
	...
	//=========================================================================
	// switch_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends switch_driver_callback;
	...
	class switch_driver_callback extends uvm_callback;
		...
		virtual function void pre_driver(switch_driver drv, switch_trans tr);
		endfunction
