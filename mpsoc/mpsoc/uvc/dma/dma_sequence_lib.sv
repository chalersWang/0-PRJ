`ifndef _DMA_SEQUENCE_LIB_SV_
`define _DMA_SEQUENCE_LIB_SV_

//=========================================================================
// dma_base_sequence: dma UVC 的基类 sequence
//   继承自 uvm_sequence，所有 dma 相关 sequence 应继承此类
//=========================================================================
class dma_base_sequence extends uvm_sequence#(dma_trans);
	...
	virtual task pre_body();
		...
		// 获取 config（若不需要可注释）
		if (!uvm_config_db#(dma_config)::get(null, get_full_name(), "dma_config", dma_cfg))
			`uvm_fatal(get_type_name(), "can not get dma_config object !!!")
	...
	virtual task body();
		...
		//
		// 方式3: 手动控制（需要精细控制时）
		// req = dma_trans::type_id::create("req");
		// start_item(req);
...
//=========================================================================
// dma_demo_sequence: 示例 sequence
//   生成单笔 transaction 并发送
//=========================================================================
class dma_demo_sequence extends dma_base_sequence;
	`uvm_object_utils(dma_demo_sequence)
	function new(string name="dma_demo_sequence");
		super.new(name);
	endfunction
	virtual task body();
