`ifndef _DMA_AGENT_SV_
`define _DMA_AGENT_SV_

//=========================================================================
// dma_agent: dma UVC 的顶层 agent
...
//	`uvm_register_cb(dma_agent, dma_agent_callback)

	`uvm_component_utils_begin(dma_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(dma_drv,  UVM_ALL_ON)
		`uvm_field_object(dma_mon,  UVM_ALL_ON)
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    ...
		    dma_drv.rsp_port.connect(dma_seqr.rsp_export);
		end
...
endclass : dma_agent
	...
	//=========================================================================
	// dma_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class dma_agent_callback extends uvm_callback;
		`uvm_object_utils(dma_agent_callback)
		function new(string name="dma_agent_callback");
			super.new(name);
		endfunction
	endclass : dma_agent_callback
*/
