`ifndef _GMAC_DRIVER_SV_
`define _GMAC_DRIVER_SV_

//=========================================================================
// gmac_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class gmac_driver extends uvm_driver#(gmac_trans);

	virtual gmac_vif    vif;
	// driver callback 池，允许用户注册回调扩展 driver 行为
//	`uvm_register_cb(gmac_driver, gmac_driver_callback)

	`uvm_component_utils(gmac_driver)

	function new(string name="gmac_driver",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual gmac_vif)::get(this,"","gmac_vif",vif))
		    `uvm_fatal("gmac_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	virtual task reset_phase(uvm_phase phase);
		...
		vif.gmac_rgmii_rxc = 'd0;
		vif.gmac_rgmii_rxd = 'd0;
		vif.gmac_rgmii_rx_ctl = 'd0;
	...
	virtual task main_phase(uvm_phase phase);
		...
		while(1) begin
			...
			if (req == null) begin
				...
				// 调用回调：pre_driver
//				`uvm_do_callbacks(gmac_driver, gmac_driver_callback, pre_driver(this, req))
				// --- 驱动 transaction ---
				driver_one_pkt(req);
				// --- 驱动完成 ---
				// 调用回调：post_driver
//				`uvm_do_callbacks(gmac_driver, gmac_driver_callback, post_driver(this, req))
	...
	//=========================================================================
	virtual task driver_one_pkt(gmac_trans tr);
...
endclass : gmac_driver

	//=========================================================================
	// gmac_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends gmac_driver_callback;
	//       function void pre_driver(...); ... endfunction
	//     endclass
	//     gmac_driver_callback::add(drv, my_cb);
	//=========================================================================
/*
	class gmac_driver_callback extends uvm_callback;
		`uvm_object_utils(gmac_driver_callback)
		function new(string name="gmac_driver_callback");
			super.new(name);
		endfunction

		// 在 driver 驱动前调用，可修改 transaction 内容
		virtual function void pre_driver(gmac_driver drv, gmac_trans tr);
		endfunction

		// 在 driver 驱动后调用，可检查驱动结果
		virtual function void post_driver(gmac_driver drv, gmac_trans tr);
		endfunction
	endclass : gmac_driver_callback
*/
