`ifndef _GMAC_AGENT_SV_
`define _GMAC_AGENT_SV_

//=========================================================================
// gmac_agent: gmac UVC 的顶层 agent
//   UVM_ACTIVE: 例化 driver + sequencer + monitor
//   UVM_PASSIVE: 仅例化 monitor
//   内建 agent_callback，支持 agent 级扩展
//=========================================================================
class gmac_agent extends uvm_agent;

	// is_active 控制 agent 工作模式（active=可驱动，passive=纯监控）
	uvm_active_passive_enum is_active = UVM_ACTIVE;

	// 子组件句柄
	gmac_driver    gmac_drv;
	gmac_sequencer gmac_seqr;
	gmac_monitor   gmac_mon;

	// agent callback 池（供用户注册扩展）
//	`uvm_register_cb(gmac_agent, gmac_agent_callback)

	`uvm_component_utils_begin(gmac_agent)
		`uvm_field_enum(uvm_active_passive_enum, is_active, UVM_ALL_ON)
		`uvm_field_object(gmac_drv,  UVM_ALL_ON)
		`uvm_field_object(gmac_mon,  UVM_ALL_ON)
		`uvm_field_object(gmac_seqr, UVM_ALL_ON)
	`uvm_component_utils_end

	function new(string name="gmac_agent",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    gmac_drv  = gmac_driver::type_id::create("gmac_drv", this);
		    gmac_seqr = gmac_sequencer::type_id::create("gmac_seqr", this);
		end
		gmac_mon = gmac_monitor::type_id::create("gmac_mon", this);
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		if(is_active==UVM_ACTIVE)begin
		    gmac_drv.seq_item_port.connect(gmac_seqr.seq_item_export);
		    // rsp_port 用于 driver→sequencer 的响应回传
		    gmac_drv.rsp_port.connect(gmac_seqr.rsp_export);
		end
...
endclass : gmac_agent
	...
	//=========================================================================
	// gmac_agent_callback: Agent 回调基类
	//   当需要在 agent 层面注入行为（如全局错误注入、协议拦截等）时使用
	//=========================================================================
/*
	class gmac_agent_callback extends uvm_callback;
		`uvm_object_utils(gmac_agent_callback)
		function new(string name="gmac_agent_callback");
			super.new(name);
		endfunction
	endclass : gmac_agent_callback
*/
