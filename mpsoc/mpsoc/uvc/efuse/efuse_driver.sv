`ifndef _EFUSE_DRIVER_SV_
`define _EFUSE_DRIVER_SV_

//=========================================================================
// efuse_driver: 将 transaction 驱动到 DUT 接口（时序级）
//   继承自 uvm_driver，通过 seq_item_port 从 sequencer 获取 transaction
//   main_phase 中运行驱动循环，reset_phase 中复位所有信号
//=========================================================================
class efuse_driver extends uvm_driver#(efuse_trans);
	...
	function new(string name="efuse_driver",uvm_component parent=null);
	...
	virtual function void build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(virtual efuse_vif)::get(this,"","efuse_vif",vif))
		    `uvm_fatal("efuse_driver","virtual interface must be set for it!!!")
	...
	*/
	...
	//=========================================================================
	virtual task driver_one_pkt(efuse_trans tr);
...
endclass : efuse_driver

	//=========================================================================
	// efuse_driver_callback: Driver 回调基类
	//   用户可继承此类扩展 driver 行为（如错误注入、协议检查等）
	//   使用方式：
	//     class my_driver_cb extends efuse_driver_callback;
	...
	class efuse_driver_callback extends uvm_callback;
		...
		virtual function void pre_driver(efuse_driver drv, efuse_trans tr);
		endfunction
